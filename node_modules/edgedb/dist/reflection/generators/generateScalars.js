    ...
    for (const type of types.values()) {
        ...
        if (type.castOnlyType) {
            ...
            sc.writeln([
                (0, builders_1.t) `export `,
                (0, builders_1.dts) `declare `,
                (0, builders_1.t) `type ${ref} = $.ScalarType<"${mapped.name}", ${tsType}, ${getTypes_1.nonCastableTypes.has(type.id) ? "false" : "true"}>;`,
            ]);
            sc.writeln([
                (0, builders_1.dts) `declare `,
                ...(0, genutil_1.frag) `const ${literal}`,
            ...
            sc.writeln([
                (0, builders_1.t) `export `,
                (0, builders_1.dts) `declare `,
                (0, builders_1.t) `type ${ref} = $.ScalarType<"${type.name}", ${tsType}, ${getTypes_1.nonCastableTypes.has(type.id) ? "false" : "true"}>;`,
            ]);
            sc.writeln([
                (0, builders_1.dts) `declare `,
                ...(0, genutil_1.frag) `const ${literal}`,
